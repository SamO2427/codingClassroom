<?php
	...
	if ($row[0] < 25)
	{
		$sql = "UPDATE account SET progress=25 WHERE username = '{$_SESSION['login_user']}'";
	}
...
<html>
	...
	<body>
		<div id="notes">
			<p>
				When storing a set of values, for example, the numbers of apples of 3 different sizes, we can make 3 variables
			</p>
			<p>
				But, we can also use an array to store the 3 values and we can make an array with the code:
			</p>
			<p>
				int apple[3];
			</p>
			<p>
				int is type of the array, apple is name of array and 3[] is size of array we want
			</p>
			<p>
				We can then put the value in to it
			</p>
			<p>
				apple[0]= 13; apple[1]=11; apple[2]=16;
			</p>
			<p>
				0,1,2 are not size of array but the index of things inside array
			</p>
			<p>The index start from 0 and end with size-1</p>
			<p>In the above example, the index end with 3-1 = 2</p>
		</div>
		...
		<div id="question">
			...
			<p>
				int a[16];
			<p>What is the index of array end with?</p>
